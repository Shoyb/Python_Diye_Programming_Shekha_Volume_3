...
def test_append():
    ...
    assert first_node.next.data == second_item, "first node should point to the second node"
...
def test_prepend():
    ...
    assert old_node.prev == new_node, "previous node of old_node should be new_node"
...
def test_search():
    ...
    dll.append(15)
    node = dll.search(10)
    assert node.data == 10, "item should be found in the list"
...
def test_remove_node():
    ...
    dll.append(5)
    node = dll.search(5)
    assert dll.head.next == node, "head should point to the node"
...
def test_remove():
    ...
    assert node_20.prev == node_10, "now previous of 20 should be 10"
    assert str(dll) == "5,10,20", "15 should not be in the list anymore"
    
    # Remove from end
    dll.remove(20)
    assert node_10.next == None, "now next of 10 should be None"
    assert str(dll) == "5,10", "20 should not be in the list anymore"
